function [dX] = spearheadCruiseModel(k,X,U,adb,l_elevon,r_elevon,rudder)
%% Xdot = f(X,U)
...
%% U = [u5 u6 u7 u8] K = Motor Gain Mau = Time Constant Ku = Input Gain
...
%% Motor Dynamics: dX = [w5dot], U = Pulse Width of the pwm signal 0-1000

dX13 = -(1/Mtau(2))*w5 + Ku(2)*U(5);

%% Servo Dynamics: dX = [w6dot w7dot w8dot], U = Pulse Width of the pwm signal 0-1000
dX14 = -(1/Mtau(3))*dl + Ku(3)*U(6);
dX15 = -(1/Mtau(3))*dr + Ku(3)*U(7);
dX16 = -(1/Mtau(3))*drd + Ku(3)*U(8);
